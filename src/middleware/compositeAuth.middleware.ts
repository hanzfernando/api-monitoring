...
class CompositeAuthMiddleware {
  ...
  // Allow credentialId to be null to avoid non-null assertions at call sites
  private static setAuthContext(req: Request, type: "apiKey" | "jwt", user: any, credentialId: string | number | null, scopes: string[] = []) {
    req.user = user;
    req.auth = { type, credentialId: credentialId != null ? String(credentialId) : null, scopes } as any;
  }
  ...
  static async either(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      if (header) {
        ...
        CompositeAuthMiddleware.setAuthContext(
          req,
          "apiKey",
          req.user,
          // avoid non-null assertion: prefer optional chaining
          req.apiKey?.id ?? null,
          req.apiKey?.scopes ?? []
        );
        return next();
      ...
      CompositeAuthMiddleware.setAuthContext(req, "jwt", req.user, req.user?.id ?? null);
      return next();
    } catch (err) {
