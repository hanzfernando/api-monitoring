...
class DatabaseManager {
  ...
  private static setupConnectionMonitoring(): void {
    ...
    DatabaseManager.instance.$connect().then(() => {
      DatabaseManager.connectionCount++;
      console.log(`Database connected successfully. Active connections: ${DatabaseManager.connectionCount}`);
    }).catch((error) => {
  ...
  public static async getConnectionPoolStats(): Promise<{
    ...
    try {
      ...
      console.log('Failed to get connection pool stats', { error });
      return {
        activeConnections: 0,
        idleConnections: 0,
        totalConnections: 0,
  ...
  public static async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; responseTime: number }> {
    ...
    try {
      ...
      const responseTime = Date.now() - start;
      console.log('Database health check failed', { error, responseTime });
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        if (attempt > maxRetries) {
          console.log(`Database operation failed after ${maxRetries} retries`, {
            error: error.message,
            code: error.code,
            attempts: attempt
          });
        ...
        console.log(`Database operation failed, retrying in ${jitterDelay}ms`, {
          error: error.message,
          attempt,
          maxRetries,
   ...
   */
  public static async executeRawQuery<T = any>(query: string, ...params: any[]): Promise<T> {
    return DatabaseManager.executeWithRetry(async () => {
      const start = Date.now();
      try {
        const result = await DatabaseManager.instance.$queryRawUnsafe(query, ...params);
        const duration = Date.now() - start;

        if (duration > DatabaseManager.slowQueryThreshold) {
          console.log('Slow raw query detected', {
        ...
        const duration = Date.now() - start;
        console.log('Raw query execution failed', {
          query: query.substring(0, 200) + (query.length > 200 ? '...' : ''),
          duration: `${duration}ms`,
          error: error.message,
          code: error.code
